        ...
        if subject and study_path:
            ...
            for sub in pass_subjects:
                for teacher in sub.teachers:
                    if study_path == "school" and teacher.is_school:
                        teachers.append(teacher)
                        continue
                    if study_path == "oge" and teacher.is_oge:
                        teachers.append(teacher)
                        continue
                    if study_path == "olymps" and teacher.is_olymps:
                        teachers.append(teacher)
                        continue
            ...
            if study_path == "school":
                teachers = Teacher.query.filter_by(is_school=True).all()
            if study_path == "oge":
                teachers = Teacher.query.filter_by(is_oge=True).all()
            if study_path == "olymps":
                teachers = Teacher.query.filter_by(is_olymps=True).all()
        else:
            teachers = Teacher.query.all()
...
def answer_and_questions():
    ...
    db.session.commit()
    return render_template(
        "main/questions_and_answers.html",
        host=config.TEACHERS_HOST,
        bot_host=config.BOT_HOST,
    )
...
def invite():
    ...
    db.session.commit()
    return render_template(
        "main/invite.html", host=config.TEACHERS_HOST, bot_host=config.BOT_HOST
    )
...
def add_statistic(name):
    names = {
        "pop_up_bot": "нажатие попапа с ботом",
...
def add_comment(teacher_id):
    ...
    if form.validate_on_submit():
        ...
        teacher.feedback = round(
            sum(map(lambda x: x.feedback, teacher.comments)) / len(teacher.comments)
        )
        db.session.commit()
